...
{
    [Route("api/booking/deposit/transfer")]
    public class BookingTransferDepositController : Controller
    {
        private readonly ApplicationOptions applicationOptions;
        private readonly TransferService transferService;
        private readonly BookingService bookingService;

        public BookingTransferDepositController(
            IOptions<ApplicationOptions> applicationOptionsAccessor,
            TransferService transferServiceAccessor, 
            BookingService bookingServiceAcessor)
        {
            ...
            this.transferService = transferServiceAccessor;
            this.bookingService = bookingServiceAcessor;
        }

        [HttpPost]
        public async Task<IActionResult> CreateDepositTransferBooking([FromBody]CreateTransferBookingModel createTransferBookingModel)
        {
            ...
            var depositPrice = totalPrice / 5;

            var externalId = Guid.NewGuid();

            var createDepositTransferBooking = new CreateDepositTransferBooking
            {
                Date = date,
                ...
                ToBeCharged = totalPrice - depositPrice,
                ApplicationId = applicationOptions.ApplicationId
            };

            await bookingService.CreateDepositTransferBookingAsync(createDepositTransferBooking);
